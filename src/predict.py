...
def setup_environment():
    ...
    logging.info(
        "Setup complete. MLflow tracking set to mlruns, "
        "predictions will be saved to results/."
    )
...
# Purpose: Preprocess the input data to match the training data format
# (e.g., scale features).
...
def load_model(
    ...
    run_id="d7912c083a16407fbe6faf6084698758"
):
...
def make_predictions(model, X, customer_ids):
    ...
    try:
        ...
        result_df = pd.DataFrame(
            {"CustomerId": customer_ids["CustomerId"], "is_high_risk_pred": predictions}
        )
        logging.info(
            "Predictions completed. First 5 predictions:\n%s", result_df.head()
        )
        return result_df
    except Exception as e:

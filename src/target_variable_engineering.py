...
# Purpose: Create the output directory for the processed dataset with the new target variable.
...
# Purpose: Compute Recency, Frequency, and Monetary values per CustomerId to capture transaction behavior.
def calculate_rfm_metrics(df):
    """Calculate RFM metrics (Recency, Frequency, Monetary) at CustomerId level."""
...
# Purpose: Scale RFM features to ensure fair clustering, as they have different scales.
...
# Purpose: Use K-Means to segment customers into 3 groups based on RFM profiles.
...
# Purpose: Analyze clusters to find the high-risk group (low Frequency and Monetary) and assign is_high_risk labels.
...
# Purpose: Integrate is_high_risk into the processed dataset from Task 3.
...
# Purpose: Save the updated dataset with the is_high_risk column for modeling.
...
def main():
    """Main function to engineer the proxy target variable for Bati Bank's credit risk model."""
    print("Starting Proxy Target Variable Engineering for Bati Bank's Credit Risk Model...")
    ...
    save_processed_data(merged_df, 'data/processed/processed_transactions_with_target.csv')

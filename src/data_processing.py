...
# Purpose: Create the output directory for processed data to ensure files
# are saved correctly.
...
# Purpose: Identify and fill missing values to ensure data quality for
# feature engineering.
...
# Purpose: Cap outliers in numerical features like Amount to reduce their
# impact on the model.
...
# Purpose: Calculate Recency, Frequency, and Monetary metrics per customer
# to capture their transaction behavior.
...
# Purpose: Add features like transaction type, category counts, and
# average transaction amounts to enhance model inputs.
...
def create_additional_features(df):
    ...
    category_counts = df.groupby(['CustomerId', 'ProductCategory'])[
        'TransactionId'].count().unstack(fill_value=0)
    ...
    ).reset_index(name='AvgTransactionAmount')
